...
`~/.parley/speak.log` records sanitized operational events, message lengths,
byte counts, and timings—not the text you dictate or the text spoken back,
voice/model names, paths, labels, credentials, or raw provider diagnostics.
Speech failures are allow-listed entries in the shared runtime status snapshot;
there is no competing error marker.
